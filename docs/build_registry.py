...
def _collision_rows() -> list[dict[str, str]]:
    """Installed entry points that name a built-in, which discovery refuses and skips.

    `plugins.discover` warns and lets the built-in stand; the registry's job is to make the
    attempt visible rather than to repeat the warning. Silent skipping is how a shadowing
    plug-in became indistinguishable from one that was never installed.
    """
    rows = []
    for ep in _entry_points(PACK_GROUP):
        if ep.name in list_packs():
            rows.append({"group": PACK_GROUP, "name": ep.name, "origin": _origin(ep)})
    for ep in _entry_points(ENGINE_GROUP):
        if ep.name in BUILTIN_ENGINE_NAMES:
            rows.append({"group": ENGINE_GROUP, "name": ep.name, "origin": _origin(ep)})
    return rows
...
def build() -> str:
    packs, engines, collisions = _pack_rows(), _engine_rows(), _collision_rows()
    pack_html = "\n".join(
    ...
    if collisions:
        collision_rows = "\n".join(
            "<tr>" + "".join(_cell(row[k]) for k in ("group", "name", "origin")) + "</tr>"
            for row in collisions
        )
        collision_html = (
            "<table><thead><tr><th>Entry-point group</th><th>Name</th><th>Origin</th></tr>"
            f"</thead><tbody>{collision_rows}</tbody></table>"
            "<p>Discovery refuses a name that shadows a built-in: the built-in stands and the "
            "entry point is skipped with a warning (<code>plugins.discover</code>).</p>"
        )
    else:
        collision_html = (
            "<p>No name collisions: no installed entry point names a built-in pack or engine.</p>"
        )
...
<h2>Name collisions</h2>{collision_html}
